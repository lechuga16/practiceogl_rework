...
public OnPluginStart() {
	...
	// Wave interval
	hCvarWaveInterval = CreateConVar("siws_wave_interval", "6", "Interval in seconds between special infected waves");
	
	// Custom class limits
	hCvarMaxSpecials 	= CreateConVar("siws_max_specials", 	"2", "Maximum Specials alive at any time");
	...
	hCvarHunterLimit 	= CreateConVar("siws_hunter_limit", 	"2", "Maximum hunters alive at any time");
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype) {
	...
	new Float:fTankHealthPercent = 100.0 * (fTankCurrentHealth/fTankMaxHealth);
...
Float:GetAverageSurvivorFlow() {
	...
	return (totalFlow/float(survivorCount));
}
