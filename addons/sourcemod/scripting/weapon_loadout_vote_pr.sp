...
{
	name = "Weapon Loadout",
	author = "Sir, A1m`, modified by lechuga",
...
void InitMenu()
{
	...
	g_hMenu.SetTitle("Select your weapon");
...
public Action Cmd_VoteMode(int iClient, int iArgs)
{
	...
	if (!IsInReady() || InSecondHalfOfRound()) {
		CPrintToChat(iClient, "{blue}[{green}!{blue}]{default}: You can only call for the vote during the first ready-up of a round");
		return Plugin_Handled;
	}
	...
	if (ReadyPlayers() != GetMaxPlayers()) {
		CPrintToChat(iClient, "{blue}[{green}!{blue}]{default}: Both teams need to be full.");
		return Plugin_Handled;
	}
		...
		if (item_info[i][BUILTINVOTEINFO_ITEM_INDEX] == BUILTINVOTES_VOTE_YES) {
			if (item_info[i][BUILTINVOTEINFO_ITEM_VOTES] > (num_clients / 2)) {
				...
				if (!IsInReady() && g_iCurrentMode != eUndecided && !g_bIsAdminVote) {
					...
					CPrintToChatAll("{blue}[{green}!{blue}]{default}: Vote didn't pass before you left ready-up.");
					return;
				}
...
public Action Timer_InformPlayers(Handle hTimer)
{
	...
	for (int i = 1; i <= MaxClients; i++) {
		if (IsClientInGame(i) && GetClientTeam(i) != L4D2Team_Spectator && !g_bVoteUnderstood[i]) {
			// CPrintToChat(i, "{blue}[{green}!{blue}]{default}: Welcome to {blue}Zone{green}Hunters{default}.");
			CPrintToChat(i, "{blue}[{green}!{blue}]{default}: Type {olive}!mode {default}in chat to vote on weapons used.");
		}
	}
